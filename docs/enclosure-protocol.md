...
| `enclosure.reset` | none | `reset()` | Restore the enclosure to its started state (eyes open, mouth at default). |

### System

| Message type | Data | Producer method | Meaning |
|---|---|---|---|
| `enclosure.system.reset` | none | `system_reset()` | Reset enclosure hardware CPUs etc. |
| `enclosure.system.mute` | none | `system_mute()` | Mute (turn off) the system speaker. |
| `enclosure.system.unmute` | none | `system_unmute()` | Unmute (turn on) the system speaker. |
...
| Message type | Data | Producer method | Meaning |
|---|---|---|---|
| `enclosure.eyes.on` | none | `eyes_on()` | Illuminate / show the eyes. |
| `enclosure.eyes.off` | none | `eyes_off()` | Turn off / hide the eyes. |
| `enclosure.eyes.blink` | `{side}` | `eyes_blink(side)` | Blink eyes. `side` is `'r'`, `'l'` or `'b'`. |
| `enclosure.eyes.narrow` | none | `eyes_narrow()` | Narrow the eyes (squint). |
...
| `enclosure.eyes.reset` | none | `eyes_reset()` | Restore eyes to the default (ready) state. |
| `enclosure.eyes.spin` | none | `eyes_spin()` | Make the eyes roll. |
...
| `enclosure.eyes.rgb.get` | none | `get_eyes_color()` / `get_eyes_pixel_color(idx)` | Request the current eye pixel colors. Expects a response on `enclosure.eyes.rgb`. |
...
| Message type | Data | Producer method | Meaning |
|---|---|---|---|
| `enclosure.mouth.reset` | none | `mouth_reset()` | Restore the mouth display to blank. |
| `enclosure.mouth.talk` | none | `mouth_talk()` | Generic talking animation (non-synched speech). |
| `enclosure.mouth.think` | none | `mouth_think()` | Thinking image / animation. |
| `enclosure.mouth.listen` | none | `mouth_listen()` | Listening image / animation. |
| `enclosure.mouth.smile` | none | `mouth_smile()` | Smile image / animation. |
...
| `enclosure.mouth.events.activate` | none | `activate_mouth_events()` | Enable mouth movement with speech. |
| `enclosure.mouth.events.deactivate` | none | `deactivate_mouth_events()` | Disable mouth movement with speech. |
